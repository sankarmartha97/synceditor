...
const { setupPageHandlers } = require('./page.handler');
...
const setupSocketHandlers = (io) => {
  ...
  io.on('connection', (socket) => {
    ...
    // ============================================
    setupPageHandlers(io, socket);

    socket.on(CLIENT_EVENTS.CANVAS_JOIN, async (data) => {
      try {
        const { canvasId } = data;
    ...
    socket.on(CLIENT_EVENTS.CANVAS_LEAVE, async (data) => {
      try {
        const { canvasId } = data;
    ...
    socket.on(CLIENT_EVENTS.WIDGET_ADD, async (data) => {
      try {
    ...
    socket.on(CLIENT_EVENTS.WIDGET_UPDATE, async (data) => {
      try {
    ...
    socket.on(CLIENT_EVENTS.WIDGET_DELETE, async (data) => {
      try {
    ...
    socket.on(CLIENT_EVENTS.CURSOR_MOVE, (data) => {
    ...
    socket.on(CLIENT_EVENTS.CURSOR_HIDE, (data) => {
      const { canvasId } = data;
